...
echo Now installing Linux kernel headers

sudo apt-get -y install linux-headers-generic || { echo "Could not install Linux kernel headers"; exit 1;}

echo Now adding Ubuntu xenial source repositories
...
deb-src http://security.ubuntu.com/ubuntu xenial-security main restricted" > \
      /etc/apt/sources.list.d/official-source-repositories.list' || { echo "Could not add Ubuntu xenial source repositories"; exit 1;}

echo Now updating apt

sudo apt-get update || { echo "Could not update apt"; exit 1;}

echo Now installing / building ROS RealSense library 

sudo apt-get -y install ros-kinetic-librealsense || { echo "Could not install / build ROS RealSense library"; exit 1;}

echo Now installing ROS RealSense camera driver 

sudo apt-get -y install ros-kinetic-realsense-camera || { echo "Could not install ROS RealSense camera driver"; exit 1;}
...
#fix for amcl
sudo cp /opt/ros/kinetic/share/turtlebot_navigation/launch/includes/amcl/amcl.launch.xml /opt/ros/kinetic/share/turtlebot_navigation/launch/includes

#fix for gmapping
sudo cp /opt/ros/kinetic/share/turtlebot_navigation/launch/includes/gmapping/gmapping.launch.xml /opt/ros/kinetic/share/turtlebot_navigation/launch/includes
